"""
Phase 5: YOLO Failure Regions (Exclusion Zones) Annotation Tool

Interactive tool to mark THE PRIMARY area where YOLO fails to detect vehicles
but they are visible to humans. These regions will be cropped and used to train
a secondary congestion detection model.

IMPORTANT: Each camera has exactly ONE problem region polygon.
...
Purpose:
- Mark the primary region that fills with cars during peak congestion
- Crop this region separately (Phase 6)
- Train a binary model to detect traffic in YOLO-blind areas
- Supplement YOLO with additional congestion signals

Controls:
- Left-click: Add vertex to failure region polygon
- Right-click: Remove last vertex
- [R] Reset polygon (start over)
...
Note: Only ONE polygon per camera (removed [C] and [D] for multiple polygons)
"""
...
class ExclusionZonesTool(PipelinePhase):
    """
    Phase 5: Interactive YOLO failure regions (exclusion zones) annotation tool.
    ...
    def __init__(self, pipeline_config: PipelineConfig):
        ...
        self.binary_labeled_dir = self.config.base_dir / self.config.binary_labeled_dir  # Load from reviewed images
        self.output_path = self.config.base_dir / "yolo_failure_regions.json"

        # GUI state (single polygon per camera)
        self.current_polygon: List[Tuple[int, int]] = []
        self.completed_polygon: Optional[List[Tuple[int, int]]] = None  # Changed from list to single polygon
    ...
    def run(self, resume: bool = False) -> Dict[str, Any]:
        ...
        while current_idx < len(camera_ids):
            ...
            # Reset annotation state (single polygon)
            self.current_polygon = []
            self.completed_polygon = None

            # Main annotation loop for this camera
            action = self._annotate_camera()

            if action == 'next':
                # Save failure region for this camera (single polygon)
                failure_regions = []
                if self.completed_polygon:
                    failure_regions = [
                        {'polygon': self.completed_polygon, 'description': 'Primary YOLO-blind area'}
                    ]
                ...
                region_count = len(failure_regions)
                self.logger.info(f"Saved {region_count} failure region for {camera_id}")
    ...
    def _annotate_camera(self) -> str:
        ...
        while True:
            ...
            if key == ord('n'):  # Next
                # Auto-complete polygon if valid
                if self.current_polygon and len(self.current_polygon) >= 3:
                    self.completed_polygon = self.current_polygon.copy()
                    self.logger.info(f"Completed polygon with {len(self.current_polygon)} vertices")
                    return 'next'
                elif self.completed_polygon:
                    # Already have completed polygon
                    return 'next'
                else:
                    self.logger.warning("No failure region drawn. Use [S] to skip if YOLO works well everywhere.")
                    continue
                ...
                self.current_polygon = []
                self.completed_polygon = None
                self.logger.info("Reset polygon")
    ...
    def _render_display(self):
        ...
        # Draw completed failure region polygon (red, filled with transparency) - SINGLE POLYGON
        if self.completed_polygon:
            poly_np = np.array(self.completed_polygon, dtype=np.int32)
            overlay = display.copy()
        ...
        # Draw current polygon being drawn (orange)
    ...
    def _draw_instructions(self, image: np.ndarray):
        ...
        region_status = "Complete" if self.completed_polygon else "Not drawn"
        instructions = [
            f"Camera: {self.current_camera_id}",
            f"Failure region status: {region_status}",
            f"Current polygon vertices: {len(self.current_polygon)}",
            "",
            "Mark PRIMARY area where: Cars visible but YOLO missed them",
            "Left-click: Add vertex | Right-click: Remove last",
            "[R] Reset polygon | [N] Next (auto-completes if valid)",
            "[S] Skip (YOLO works well) | [Q] Quit",
            "[V] Toggle YOLO boxes | [L] Toggle lane polygon",
            "",
            "ONE polygon per camera (3+ vertices required)",
        ]
...
def main():
    ...
    phase = ExclusionZonesTool(config)
